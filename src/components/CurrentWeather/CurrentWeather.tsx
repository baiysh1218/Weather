...
const CurrentWeather = () => {
  const { current, location, forecast } = useAppSelector(
  ...
  const handleCheckCondition = () => {
    const dataTime = forecast?.forecastday[0].astro;
    ...
    if (splitedTime) {
      ...
      if (current?.condition.text.includes("Солнечно")) {
        return (
          <svg
            width="119"
            height="119"
      ...
      } else if (current?.condition.text.includes("дождь")) {
        return (
          <svg
            width="119"
            height="119"
  ...
  return (
    <div className="daily_weather">
      <div className="daily_f_info">
        <h2>{current ? Math.floor(current.temp_c) : 0}°</h2>
        <h3>Сегодня</h3>
        <p className="daily_f_info_item daily_f_info_item_p">
          Время: {splitedLocation}
        </p>
        <p className="daily_f_info_item">Город: {location?.region}</p>
      </div>
      <div className="daily_img">{condition}</div>
    </div>
  );
};
